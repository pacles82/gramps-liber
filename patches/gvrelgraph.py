...
class RelGraphReport(Report):
    ...
    def __add_family(self, fam_handle):
        ...
        if self.event_choice == 7:
            ...
            if d_type:
                ...
                if d_date and d_place:
                    ...
                    line = ""
                if d_place:
                    line += " %s" % self.get_place_with_coa(d_place)
                labellines.append(line)
        else:
            if m_date:
            ...
            if d_date:
                labellines.append("(%s)" % d_date)
            if d_place:
                labellines.append("(%s)" % self.get_place_with_coa(d_place))
        ...
        # If coats of arms were collected, render them as an HTML table for Graphviz
        if self.include_coatsofarms and getattr(self, "_coa_rows", None):
            html_label = '<TABLE BORDER="0" CELLSPACING="2" CELLPADDING="0" CELLBORDER="0"><TR><TD>%s</TD></TR>' % label.replace("\\n", "<BR/>").replace("\n", "<BR/>")
            html_label += "<TR>"
    ...
    def get_person_label(self, person):
        ...
        if self.event_choice != 0:
            ...
            if (
                ...
                label += ")"
            # Residence with coat of arms (if enabled, same event_choice that shows places)
